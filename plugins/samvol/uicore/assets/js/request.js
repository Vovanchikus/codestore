(function (root, factory) {
    ...
    return function install(UI, Snowboard) {
        const ensureSnowboard = () => {
            if (!Snowboard || typeof Snowboard.request !== 'function') {
                throw new Error('Snowboard.request is required. Include modules/system/assets/js/snowboard/request.js.');
            }
        };
        ...
        const buildOptions = (options = {}) => {
            ...
            safeOptions.success = (response, requestInstance) => {
                ...
                if (typeof userSuccess === 'function' && userSuccess(response, requestInstance) === false) {
                    return false;
                }

                if (response && response.success === false) {
                    if (UI.toast) {
                        UI.toast.error(response.message || 'Server rejected the request.');
                    }
                    return false;
                }
            ...
            safeOptions.error = (errorResponse, requestInstance) => {
                emit('request:error', { error: errorResponse, request: requestInstance });

                if (typeof userError === 'function' && userError(errorResponse, requestInstance) === false) {
                    return false;
                }
        ...
        const request = (element, handler, options = {}) => {
            ...
            if (typeof element === 'string' && element.indexOf('on') === 0 && handler === undefined) {
                options = handler || {};
                handler = element;
                element = null;
            }
